...
$titulo = "Parametrización de Preguntas de preguntas_entrevistas"; // Título específico para esta vista
    ...
    <div class="main">
        ...
        <main class="content">
            <div class="container-fluid p-0">
                <div class="mb-3">
                    <h1 class="h3 d-inline align-middle"><i class="align-middle me-2 fas fa-fw fa-cogs"></i>Parametrización de Preguntas de Entrevistas</h1>
                    ...
                    <div class="col-12">
                        <div class="card">
                            <div class="card-header">
                                <h5 class="card-title">Crea/Edita/Elimina las Parametrización de Preguntas de Entrevistas</h5>
                            </div>
                            <div class="card-body">
                    ...
                    <div class="modal-dialog">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title" id="modalLabel"><i class="align-middle me-2 far fa-fw fa-folder"></i>Nueva preguntas_entrevistas</h5>
...
<script>
    $(document).ready(function() {
        var table = $('#datosTabla').DataTable({
            "ajax": "<?= API ?>preguntas_entrevistas.php?action=fetch",
            "columns": [
                { "data": "id" },
                { "data": "pregunta" },
                {
        ...
        // Agregar o editar preguntas_entrevistas
        $('#formDatosCrud').submit(function(e) {
            e.preventDefault();
            var formData = $(this).serialize();

            $.ajax({
                url: '<?= API ?>preguntas_entrevistas.php?action=save',
                type: 'POST',
                data: formData,
        ...
        $('#addNuevo').on('click', function() {
            $('#modalLabel').text('Nueva Pregunta');
        ...
        $('#datosTabla tbody').on('click', '.edit', function() {
            $('#modalLabel').text('Editar Pregunta');
            var id = $(this).data('id');

            $.ajax({
                url: '<?= API ?>preguntas_entrevistas.php?action=edit&id=' + id,
                type: 'GET',
                dataType: 'json',
                ...
                error: function(xhr, status, error) {
                    ...
                    alert('Ocurrió un error al intentar cargar los datos de la Pregunta.');
                }
            });
        });
        ...
        $('#datosTabla tbody').on('click', '.delete', function() {
            if (confirm('¿Estás seguro de eliminar esta pregunta?')) {
                ...
                $.ajax({
                    url: '<?= API ?>preguntas_entrevistas.php?action=delete&id=' + id,
                    type: 'GET',
                    success: function(response) {
                        table.ajax.reload();
                    }
