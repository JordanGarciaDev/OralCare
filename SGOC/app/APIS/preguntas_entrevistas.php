...
if ($action == 'fetch') {
    $query = "SELECT * FROM preguntas_entrevistas";
...
if ($action == 'save') {
    ...
    if ($id == '') {
        // Crear nuevo
        $query = "INSERT INTO preguntas_entrevistas (pregunta) VALUES (?)";
        ...
        $query = "UPDATE preguntas_entrevistas SET pregunta = ? WHERE id = ?";
        $stmt = $conn->prepare($query);
...
if ($action == 'edit') {
    $id = $_GET['id'];
    $query = "SELECT * FROM preguntas_entrevistas WHERE id = ?";
    $stmt = $conn->prepare($query);
