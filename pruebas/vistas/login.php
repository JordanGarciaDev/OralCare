...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $query = "SELECT *,c.nombre AS nomcargo, u.id AS idUser FROM candidatos u INNER JOIN cargos c ON u.cargo_id = c.id WHERE num_doc = ?";
    ...
    if ($result->num_rows > 0) {
        ...
        if ($password == $infoUsuario['num_doc']) {
            ...
            $_SESSION['nomusuario'] = $infoUsuario['nombre_completo']; // Guardar el nombre del usuario en la sesión
        ...
        $alert = "No existe ningun candidato con estas credenciales";
